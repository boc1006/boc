package com.boc.common.utils.httpclient;
...
//				}
	...
	public static String urlEncode(String url, String charSet){
		...
		String builderQueryString = "";
		StringBuilder newQueryString =new StringBuilder(200);
		if(queryString.length()>0){
			...
			for(String nameValue : nameValues){
				...
				newQueryString.append(pname).append("=").append(pvalue).append("&");
			}
			builderQueryString = newQueryString.substring(0, newQueryString.length()-1);
		}
		return serviceUrl + "?" + builderQueryString;
	}
	
	public static String mapToQueryString(Map parameters, String charSet) {
		String queryString = "";
		StringBuilder builderQueryString =new StringBuilder(200);
		if (parameters!=null && !parameters.isEmpty()) {
			...
			for(Entry entry : entrySet){
				try {
					...
					for(Object v : values){
						builderQueryString.append(key).append("=").append(URLEncoder.encode(v==null?"":v.toString(), charSet)).append("&");
					}
				} catch (UnsupportedEncodingException e) {
					throw new IllegalArgumentException("invalid charset : " + charSet);
				}
			}
			if (builderQueryString.length() > 0) {
				queryString = builderQueryString.substring(0, builderQueryString.length()-1);
			}
		}
		return queryString;
	}
