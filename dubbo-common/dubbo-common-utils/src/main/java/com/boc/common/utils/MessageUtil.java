...
public class MessageUtil {
	...
	private static String url = "http://api.sms.testin.cn/sms";
...
//				System.out.println(
//						(new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date()) + "回复短信:" + result);
//
//			}
...
//			if (jsonReq.get("code").toString().equals("1000")) {
//				System.out.println(
//						(new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date()) + "报告回执:" + result);
//
//			}
//		} catch (JSONException e) {
//			// TODO Auto-generated catch block
...
//
//			if (jsonReq.get("code").toString().equals("1000")) {
//				System.out.println((new java.text.SimpleDateFormat("yyyy-MM-dd HH:mm:ss")).format(new Date()) + "账号余额:"
//						+ jsonReq.get("balance"));
//
//			}
//		} catch (JSONException e) {
...
//	}
	...
	public static boolean sendSmsCodeForRegister (String phone,String smsCode) {
		try {
			JSONObject jsonObj = new JSONObject();
			jsonObj.put("op", "Sms.send");
			jsonObj.put("apiKey", "fcdab35e7be67146079e8a85f40703ca");
			jsonObj.put("ts", System.currentTimeMillis());
			jsonObj.put("phone", phone);
			jsonObj.put("templateId", "1316");
			jsonObj.put("content", smsCode);
			jsonObj.put("taskId", System.currentTimeMillis());// 不超过64位长度的唯一字符串，通过和recvRPT获取的结果里的teskid关联，确定发送的信息是否收到。
			jsonObj.put("extNum", "");
			jsonObj.put("sig", getSig(jsonObj, "37CF48E33D3971D2"));
			String result = MessageUtil.transmessage(url, jsonObj.toString());
			JSONObject jo = JSONObject.fromObject(result);
			if(jo.getString("code").equals("1000")) {
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void main(String[] args) throws Exception {
		...
		jsonObj.put("apiKey", "fcdab35e7be67146079e8a85f40703ca");
		jsonObj.put("ts", System.currentTimeMillis());
		jsonObj.put("phone", "13458585957");
		jsonObj.put("templateId", "1064");
...
//		// ret is:
//		// {"op":"Sms.mo","list":[{"content":"gpo","extNum":"322123","phone":"15821030000"}],"code":1000,"msg":"成功"}
//
//		MessageUtil.recvRPT();// 接收报告 //接受回复（循环调用，有回复返回结果list，没有list返回null）
//		// ret is:
//		// {"op":"Sms.status","list":[{"taskId":"1446429441789","phone":"139107*****","status":1,"descr":"DELIVRD"},{"taskId":"1446009231400","phone":"139107*****","status":1,"descr":"DELIVRD"}],"code":1000,"msg":"成功"}
//
